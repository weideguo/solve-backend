...
if not list(redis_manage_client.scan_iter(config.key_solve_config)):
    origin_config = [
        ("job_types", "config_job_types", set(["default", "update", "test"])),
        (
            "target_types",
            "config_target_types",
        ...
        (
            "tmpl_realhost",
            "config_tmpl_realhost",
            {
                "name": "必须严格为 realhost_<ip>",
                "ip": "与管理机网络互通的ip或者<ip>_<tag> tag为任意的别名",
                "user": "ssh登录以及执行命令账号",
                "passwd": "账号密码 或 密码的加密串",
                "ssh_port": "ssh端口号",
                "proxy": "使用的代理，不用代理设置为空。",
            },
        ),
        (
            "tmpl_host",
        ...
        (
            ...
            "config_tmpl_cluster",
            {
                "name": "执行对象的名称 以cluster开头 以_分隔层级",
                "db1": "db主",
                "db2": "db从",
                "web": "游戏服",
                "site": "游戏集群的site值",
