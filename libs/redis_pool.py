...
redis_tmp_pool=redis.ConnectionPool(host=cp.get('redis_tmp','host'), port=int(cp.get('redis_tmp','port')), db=int(cp.get('redis_tmp','db')), password=cp.get('redis_tmp','passwd'), decode_responses=True)
