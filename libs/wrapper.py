...
def get_playbook_root():
    ...
    if not playbook_root:
        playbook_root = os.path.join(
            redis_send_client.hget("__solve__", "base_dir"), "playbook"
        )
...
class HashCRUD:
    ...
    def get(redis_client, request, args=None, filter_tmp=""):
        ...
        if args == "get":
            ...
            for t in target_list:
                ...
                if (
                    filter_tmp
                    and len(t.split(filter_tmp)) >= 2
                    and re.match("^[a-zA-Z0-9]{32}$", t.split(filter_tmp)[-1])
                ):
                    continue
            ...
            for k in target_list:
                if (
                    filter_tmp
                    and len(k.split(filter_tmp)) >= 2
                    and re.match("^[a-zA-Z0-9]{32}$", k.split(filter_tmp)[-1])
                ):
                    continue
    ...
    def post(redis_client, request, args=None):
        ...
        if not target:
            return Response(
                {"status": -4, "msg": safe_decode(_("name attribute must exist"))}
            )
        ...
        if target_o:
            ...
            if not list(redis_client.scan_iter(target_o)):
                return Response(
                    {"status": -3, "msg": safe_decode(_("modified target not exist"))}
                )
            elif target != target_o and list(redis_client.scan_iter(target)):
                return Response(
                    {
                        "status": -2,
                        "msg": safe_decode(_("change to target already exist")),
                    }
                )
            else:
