...
from .token_auth import (
    ...
    regexp_check,
    choice_match_check,
    single_choice_rule_check_querydict,
    single_choice_rule_check_dict,
    PermanentTokenAuthentication,
    get_token_value,
)
...
class TestPermanentTokenAuthentication(TestCase):
    ...
    def setUp(self):
        ...
        self.request.META = {
            "HTTP_AUTHORIZATION": "permanent_token 675ab0baa7f747813903e8b7dbd14de3"
        }
    ...
    def test_regexp_check(self):
        self.assertTrue(
            regexp_check("/api/v1/home/info", ["/api/v1/home/info", "/api/v2/.*"])
        )
        self.assertFalse(
            regexp_check("/api/v1/home/info", ["/x/api/v1/home/info", "/api/v2/.*"])
        )
        self.assertFalse(
            regexp_check("/api/v1/home/info", ["/x/api/v1/home/info/y", "/api/v2/.*"])
        )
        self.assertFalse(
            regexp_check("/api/v1/home/info", ["/v1/home/info", "/api/v2/.*"])
        )
        self.assertFalse(
            regexp_check("/api/v1/home/infox", ["/api/v1/home/info", "/api/v2/.*"])
        )

    def test_single_choice_rule_check_querydict(self):
        self.assertTrue(
            single_choice_rule_check_querydict(
                QueryDict("a=aaaa&c=cccc"), {"a": "aaaa", "b": "bbbb"}
            )
        )
        self.assertTrue(
            single_choice_rule_check_querydict(
                QueryDict("a=aaaa&c=cccc"), {"ab": "aaaa", "b": "bbbb"}
            )
        )
        self.assertTrue(
            single_choice_rule_check_querydict(
                QueryDict("a=aaaa&a=aaaabb&c=cccc"), {"a": "aaaa.*", "b": "bbbb"}
            )
        )
        self.assertFalse(
            single_choice_rule_check_querydict(
                QueryDict("a=aaaa&a=xxx&c=cccc"), {"a": "aaaa", "b": "bbbb"}
            )
        )
        self.assertFalse(
            single_choice_rule_check_querydict(
                QueryDict("a=aaaa&c=cccc"), {"a": "aaaaa", "b": "bbbb"}
            )
        )
        self.assertFalse(
            single_choice_rule_check_querydict(
                QueryDict("a=aaaa&b=cccc"), {"a": "aaaa", "b": "bbbb"}
            )
        )
    ...
    def test_single_choice_rule_check_dict(self):
        self.assertTrue(
            single_choice_rule_check_dict(
                {"a": "aaaa", "c": "cccc"}, {"a": "aaaa", "b": "bbbb"}
            )
        )
        self.assertTrue(
            single_choice_rule_check_dict(
                {"a": "aaaa", "c": "cccc"}, {"ab": "aaaa", "b": "bbbb"}
            )
        )
        self.assertTrue(
            single_choice_rule_check_dict(
                {"a": "aaaabb", "c": "cccc"}, {"a": "aaaa.*", "b": "bbbb"}
            )
        )
        self.assertFalse(
            single_choice_rule_check_dict(
                {"a": "aaaa", "c": "cccc"}, {"a": "aaaaa", "b": "bbbb"}
            )
        )
        self.assertFalse(
            single_choice_rule_check_dict(
                {"a": "aaaa", "b": "cccc"}, {"a": "aaaa", "b": "bbbb"}
            )
        )
    ...
    def test_choice_match_check(self):
        ...
        self.assertTrue(
            choice_match_check((QueryDict("a=aaaa&b=bbb"), [{"a": "aaaa", "v": "vvv"}]))
        )
        self.assertTrue(
            choice_match_check(
                (
                    QueryDict("a=aaaa&b=bbb"),
                    [{"a": "aaaa", "v": "vvv"}, {"a": "abbb", "v": "vvv"}],
                )
            )
        )
        self.assertFalse(
    ...
    def test_get_token_value(self):
        self.assertNotEqual(get_token_value(self.request), None)
        self.request.META = {}
        self.assertEqual(get_token_value(self.request), None)
        ...
        with self.assertRaises(exceptions.AuthenticationFailed):
            get_token_value(self.request)
        self.request.META["HTTP_AUTHORIZATION"] = (
            "permanent_token 675ab0baa7f7 47813903e8b7dbd14de3"
        )
        with self.assertRaises(exceptions.AuthenticationFailed):
            get_token_value(self.request)
