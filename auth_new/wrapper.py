...
def get_service_token(service_proxyValidate, targetService=None, verify=True):
    ...
    token_raw = (
        CASProxyToken.objects.filter(service=targetService).values("token").first()
    )
    if token_raw:
    ...
    if not token:
        pgtId_raw = (
            CASProxyPgt.objects.values("pgtId").order_by("-date_generate").first()
        )
        if pgtId_raw:
        ...
        if pgtId:
            r = requests.get(
                "%s/proxy?pgt=%s&targetService=%s" % (cas_url, pgtId, targetService),
                verify=verify,
            )
            ...
            if ("proxyTicket" in msg) and isinstance(msg, dict):
                ...
                r_token = requests.get(
                    "%s?service=%s&ticket=%s"
                    % (service_proxyValidate, targetService, proxyTicket),
                    verify=verify,
                ).text
