...
class PermanentTokenAuthentication(BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        if _permanent_token:
            permanent_token.update(
                lastest_date=datetime.datetime.now(), invoke_count=F("invoke_count") + 1
            )
        else:
            return None
        ...
        _invoke_rule_ids = (
            _permanent_token.invoke_rule_ids.split(",")
            if _permanent_token.invoke_rule_ids
            else []
        )
        ...
        permanent_token.update(
            lastest_success_date=datetime.datetime.now(),
            invoke_success_count=F("invoke_success_count") + 1,
        )
    ...
    def authenticate(self, request):
        ...
        if token_value is None:
            raise exceptions.AuthenticationFailed(
                _("temp token is not in request parameters")
            )
        ...
        if token_info:
            ...
            raise exceptions.AuthenticationFailed(
                _("temp token [%(token_value)s] is not validated")
    ...
    for invoke_rule in invoke_rules:
        ...
        must_matchs = [
            (
                request.path,
                (
                    json.loads(invoke_rule.path.strip())
                    if invoke_rule.path and invoke_rule.path.strip()
                    else []
                ),
            ),
            (
                request.method,
                (
                    json.loads(invoke_rule.method.strip())
                    if invoke_rule.method and invoke_rule.method.strip()
                    else []
                ),
            ),
            (
                from_host,
                (
                    json.loads(invoke_rule.source.strip())
                    if invoke_rule.source and invoke_rule.source.strip()
                    else []
                ),
            ),
        ]
        ...
        choice_matchs = [
            (
                request.GET,
                (
                    json.loads(invoke_rule.params.strip())
                    if invoke_rule.params and invoke_rule.params.strip()
                    else []
                ),
            ),
            (
                request.data,
                (
                    json.loads(invoke_rule.body.strip())
                    if invoke_rule.body and invoke_rule.body.strip()
                    else []
                ),
            ),
        ]
