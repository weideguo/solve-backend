...
def get_login_client():
    ...
    Account.objects.create_user(username=user, password=password, is_superuser=1)
...
class TestAuth(LoginTestCase):
    ...
    def test_userinfo_put(self):
        ...
        response = self.login_client.put(
            self.current_url, json.dumps(data), content_type="application/json"
        )
        r = response.content.decode("utf8")
        # print(r)
        r = json.loads(r)
...
def get_cas_login_url(cls):
    ...
    response = cls.client.get(
        str(reverse("auth:cas", args=("login",))) + "?service=" + cls.service
    )
    try:
...
def get_cas_ticket(cls):
    ...
    # 登陆cas，获取重定向信息
    r = requests.post(
        cls.cas_login_url, data=data, cookies=cookies, allow_redirects=False
    )
    try:
...
def cas_auth(cls, pgtUrl=""):
    ...
    if pgtUrl:
        url = cas_valid_url + "?ticket=%s&service=%s&pgtUrl=%s" % (
            ticket,
            cls.service,
            pgtUrl,
        )
...
service = "http://192.168.85.128:8080/"  # 前端的回调地址，且必须与cas的回调设置一致
proxy_callback = "https://192.168.85.128:9000/api/v1/cas/callback"  # 自己的cas回调地址，用于当前服务连接其他服务（需要为https）
another_service = "https://192.168.85.128:9000/api/v1/cas/proxyValidate"  # 要连接的其他服务的验证地址，用于当前服务连接其他服务（需要为https，且cas中设置好允许的地址）
...
class TestAuthCasProxy(BaseTestCase):
    ...
    def test_auth_other(self):
        ...
        print(
            """from auth_new.wrapper import get_service_token;get_service_token("%s",verify=False)"""
            % another_service
        )
