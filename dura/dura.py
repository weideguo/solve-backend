...
class Dura:
    ...
    def __init__(
        ...
        self.primary_key = (
            primary_key  # 存放在mongodb时的记录key名字的字段，设置为主键或唯一键
        )
                ...
                for key_config in db_config[pos]:
                    if re.match(key_config["key_pattern"], k):
                        ...
                        try:
                            ...
                            MYLOGERROR.error(
                                "save key failed-------%s %s %s %s"
                                % (k, str(redis_client), collection_name, is_update)
                            )
        ...
        if key_type == "list":
            ...
            if exist_count and is_update == -1:
                ...
                key_list = (
                    ...
                    .find({self.primary_key: key}, {self.list_name: {"$slice": -1}})[0]
                    .get(self.list_name, [])
                )
                ...
                self.db[collection_name].update_many(
                    ...
                    {"$push": {self.list_name: {"$each": new_list}}},
                )
                ...
                self.db[collection_name].update_many(
                    {self.primary_key: key}, {"$set": value}
                )
                return key, "update"
            else:
                # mongodb中记录不存在
    ...
    def load(self):
        ...
        for db_config in self.key_map_config["load"]:
            ...
            for key_config in db_config[pos]:
                ...
                MYLOGGER.info(
                    "load collection [ %s ] from mongodb to redis done"
                    % collection_name
                )
    ...
    def get_collection_name(self, key, redis_client):
        ...
        try:
            ...
            MYLOGERROR.error(format_exc())
            MYLOGERROR.error(
                "get collection name error for [ %s ] by [ %s ]"
                % (key, str(self.key_map_config["save"]))
            )
    ...
    def reload(self, key, redis_client, is_update=1):
        ...
        summary = {}
        if collection_name:
            summary = self.real_load(
                redis_client, collection_name, {self.primary_key: key}, is_update
            )
            ...
            if key_name:
                ...
                if is_update or (not key_exist):
                    ...
                    if is_error:
                        ...
                        MYLOGERROR.error(
                            "key info error, %s at collection [ %s ] out of expect"
                            % (str(key_info), collection_name)
                        )
                        summary["error"] += 1
                    else:
                        summary["update"] += 1
                ...
                MYLOGERROR.error(
                    "key info error, '%s' not in %s at collection [ %s ]"
                    % (self.primary_key, str(key_info), collection_name)
                )
                summary["error"] += 1
