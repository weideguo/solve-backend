...
class SolveDura:
    ...
    def get_keys(self, key):
        ...
        session_list = list(
            self.redis_tmp_client.scan_iter(config.prefix_session + "*" + job_id)
        )
        ...
        if "log" in log_job_info:
            ...
            for tl in ast.literal_eval(log_job_info.get("log", "[]")):
                ...
                global_list += list(
                    self.redis_tmp_client.scan_iter(config.prefix_global + target_id)
                )
                # global_target_list += self.redis_tmp_client.keys('*'+target_id)
                sum_list += list(
                    self.redis_log_client.scan_iter(config.prefix_sum + target_id)
                )
                log_list += self.redis_log_client.lrange(
                    target_log, 0, self.redis_log_client.llen(target_log)
                )
    ...
    def expire(self, key, force=0, expire_time=config.tmp_config_expire_sec):
        ...
        """
        target_log_list, sum_list, log_list, session_list, global_list, target_list = (
            self.get_keys(key)
        )

        client_key = [
            (self.redis_log_client, [key] + target_log_list + log_list + sum_list),
    ...
    def get_key_info_from_mongo(self, redis_client, key=None, pattern=None):
        ...
        if key:
            ...
            if connection_name:
                key_info = self.dura.db[connection_name].find(
                    {self.dura.primary_key: key}
                )
        elif pattern:
            connection_name = self.dura.get_collection_name(pattern, redis_client)
            if connection_name:
                key_info = self.dura.db[connection_name].find(
                    {self.dura.primary_key: {"$regex": pattern}}
                )
    ...
    def get_keys_from_mongo(self, key):
        ...
        job_info = self.get_key_info_from_mongo(
            self.redis_job_client, config.prefix_job + job_id
        )
        ...
        session_list_info = self.get_key_info_from_mongo(
            self.redis_tmp_client, pattern=session_pattern
        )
        ...
        if "log" in log_job_info:
            ...
            for tl in ast.literal_eval(log_job_info.get("log", "[]")):
                ...
                target_list_info = self.get_key_info_from_mongo(
                    self.redis_tmp_client, pattern=target_pattern
                )
                ...
                sum_list.append(
                    config.prefix_sum + target_id,
                )

                log_list_info = self.get_key_info_from_mongo(
                    self.redis_log_client, target_log
                )
                if self.dura.list_name in log_list_info and isinstance(
                    log_list_info[self.dura.list_name], list
                ):
...
cp = util.getcp()
if (
    cp.has_section("mongodb")
    and cp.has_option("mongodb", "uri")
    and cp.has_option("mongodb", "db")
):
...
if MONGODB_CONFIG:
    # 单例模式
    MYLOGGER.info(
        "durability for MONGODB_CONFIG [ %s ] in settings " % str(MONGODB_CONFIG)
    )
    solve_dura = SolveDura(MONGODB_CONFIG)
else:
    MYLOGGER.info(
        "no durability for MONGODB_CONFIG [ %s ] in settings " % str(MONGODB_CONFIG)
    )
    solve_dura = BaseDura()
