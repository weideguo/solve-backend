...
class Home(baseview.BaseView):
    ...
    def get(self, request, args=None):
        redis_config_client, redis_job_client, redis_manage_client = (
            redis_single["redis_config"],
            redis_single["redis_job"],
            redis_single["redis_manage"],
        )
        if args == "info":
            info = {}
            ...
            info["realhost"] = len(
                redis_config_client.keys(config.prefix_realhost + "*")
            )

            all_target = []

            target_types_key = redis_manage_client.hget(
                config.key_solve_config, config.target_types
            )
            ...
            real_all_target = []
            for k in all_target:
                if len(k.split(config.spliter)) >= 2 and re.match(
                    "^[a-zA-Z0-9]{32}$", k.split(config.spliter)[-1]
                ):
                    continue
            ...
            set_sort_key(
                redis_job_client, cache_key, config.prefix_job, "begin_time", True
            )
            ...
            for j in job_list:
                ...
                j_time = datetime.datetime.fromtimestamp(float(j_timestamp)).strftime(
                    "%y-%m-%d"
                )
                if j_time in stats:
            ...
            return Response(
                {
                    "status": -2,
                    "msg": util.safe_decode(_("request url not implemented")),
                }
            )
