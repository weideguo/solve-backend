            ...
            return Response(
                {
                    "status": -1,
                    "path": path,
                    "msg": util.safe_decode(_("should not change in path")),
                }
            )
                ...
                return Response(
                    {
                        "status": 1,
                        "file": filename,
                        "content": util.safe_decode(content),
                    }
                )
            except:
                return Response(
                    {
                        "status": -1,
                        "file": filename,
                        "msg": util.safe_decode(_("read file failed")),
                    }
                )

        if args == "preDownload":
            try:
                token_name = secrets.token_hex(16)
                ...
                cache.set(
                    "temp_token" + "_" + token_name,
                    token_value,
                    timeout=config.temporary_token_expire_time,
                )
                return Response({"status": 1, "temp_token": token_name})
            except:
                MYLOGERROR.error(format_exc())
                return Response(
                    {"status": -1, "msg": util.safe_decode(_("set temp token failed"))}
                )
                ...
                return Response(
                    {
                        "status": -1,
                        "path": root_path,
                        "msg": util.safe_decode(_("path is not dir")),
                    }
                )
    ...
    def get(self, request, args=None):
        ...
        if "HTTP_RANGE" in request.META:
            ...
            try:
                ...
                # "bytes=1000-2000"
                seek_pos, seek_end = [
                    int(b) for b in range_header.split("=")[1].split("-")
                ]
            except:
                MYLOGERROR.info(
                    "http header [Range] in wrong format: %s" % range_header
                )
            ...
            return Response(
                {
                    "status": -1,
                    "file": filename,
                    "msg": util.safe_decode(_("path is not file")),
                },
                status=404,
            )
