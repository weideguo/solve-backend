...
class Config(baseview.BaseView):
    ...
    def get(self, request, args=None):
        ...
        if key_name:
            ...
            return Response(
                {"status": -1, "data": "", "msg": util.safe_decode(_("key not exist"))}
            )

        data = None
        if key_type == "list":
            data = redis_manage_client.lrange(
                key_name, 0, redis_manage_client.llen(key_name)
            )
        elif key_type == "set":
            ...
            return Response(
                {
                    "status": -2,
                    "data": "",
                    "msg": util.safe_decode(_("redis key type error")),
                }
            )
    ...
    def post(self, request, args=None):
        ...
        if not info:
            return Response(
                {"status": -2, "msg": util.safe_decode(_("not commit empty info"))}
            )
        ...
        def update_config(info):
            if key_type == "list":
                ...
                return Response(
                    {"status": 1, "key": key, "type": key_type, "data": info}
                )
            elif key_type == "set":
                redis_manage_client.delete(key_name)
                for k in info:
                ...
                return Response(
                    {"status": -1, "msg": util.safe_decode(_("type constrict"))}
                )
        ...
        if key_name:
            ...
            if o_key_type == key_type or (o_key_type == "none"):
                ...
                return Response(
                    {"status": -2, "msg": util.safe_decode(_("type not match"))}
                )
        else:
