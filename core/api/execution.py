...
def get_session(pre_job_name, redis_manage_client):
    ...
    session_info = {}
    old_session_info = redis_manage_client.hgetall(
        session_tag + config.spliter + pre_job_name
    )
        ...
        if not args:
            ...
            if not data:
                return Response(
                    {
                        "status": -1,
                        "msg": util.safe_decode(_("should not commit empty data")),
                    }
                )
    ...
    def get(self, request, args=None):
        ...
        if args == "rerun":
            ...
            if _job_id:
                ...
                try:
                    session_data = redis_tmp_client.hgetall(
                        redis_tmp_client.hget(
                            target + config.spliter + target_id, config.prefix_session
                        )
                    )
                except:
                    session_data = {}
                ...
                if session_data:
                    ...
                    redis_tmp_client.expire(
                        new_session_name, config.tmp_config_expire_sec
                    )
                    job_info[config.prefix_session] = new_session_name
            ...
            if begin_line:
                redis_tmp_client.hmset(
                    target + config.spliter + new_target_id,
                    redis_tmp_client.hgetall(target + config.spliter + target_id),
                )
            else:
                try:
                    redis_tmp_client.hmset(
                        target + config.spliter + new_target_id,
                        redis_config_client.hgetall(target),
                    )
                except:
            ...
            global_data = redis_tmp_client.hgetall(
                config.prefix_global + config.spliter + target_id
            )
            if global_data:
                redis_tmp_client.hmset(
                    config.prefix_global + config.spliter + new_target_id, global_data
                )
                # 在此提前设置过期时间 防止执行到一半失败时global_xxx一直存在 命令分发后端会在执行结束后再设置一次
                redis_tmp_client.expire(
                    config.prefix_global + config.spliter + new_target_id,
                    config.tmp_config_expire_sec,
                )
            ...
            if redis_log_client.hget(config.prefix_log + old_job_id, config.job_rerun):
                rerun_str = (
                    redis_log_client.hget(
                        config.prefix_log + old_job_id, config.job_rerun
                    )
                    + ","
                    + job_name
                )
            ...
            redis_log_client.hset(
                config.prefix_log + old_job_id, config.job_rerun, rerun_str
            )
...
class ExecutionInfo(baseview.BaseView):
    ...
    def get(self, request, args=None):
        ...
        if "data" in _response and all(
            [
                isinstance(d, dict) and "playbook" in d and "target_type" in d
    ...
    def post(self, request, args=None):
        ...
        try:
            ...
            return Response(
                {
                    "status": -1,
                    "msg": util.safe_decode(_("commit date attr error")),
                    "data": data,
                }
            )
        ...
        if (
            (isinstance(parallel, str) and parallel == "true")
            or (isinstance(parallel, int) and parallel > 0)
            or (isinstance(parallel, int) and parallel)
        ):
            # 并行执行
                ...
                if next_line > current_line:
                    for i in range(next_line - current_line):
                        ...
                        redis_send_client.expire(
                            block_key, config.tmp_config_expire_sec
                        )
