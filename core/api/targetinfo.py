...
class Target(baseview.BaseView):
    ...
    def get(self, request, args=None):
        ...
        if args == "detail":
                ...
                return Response(
                    {
                        "status": -1,
                        "data": "",
                        "msg": util.safe_decode(_("target not exist")),
                    }
                )
            if target_field == "":
                    ...
                    return Response(
                        {
                            ...
                            "msg": util.safe_decode(
                                _(
                                    "get info of target [%(target_name)s] field [%(field)s] failed"
                                )
            ...
            # 其他的 get del info 操作
            return HashCRUD.get(
                redis_config_client, request, args, filter_tmp=config.spliter
            )
...
class Host(baseview.BaseView):
    ...
    def get(self, request, args=None):
        ...
        def get_online():
            ...
            for h in online_host:
                ...
                if timestamp_gap < config.host_check_success_time:
                    ...
                    is_conn = 0
                real_online_host.append(
                    {
                        "host": h.split(config.prefix_heart_beat)[1],
        ...
        if args == "online":
            ...
            return Response(
                {"status": 1, "data": real_online_list, "page": page_number}
            )
            ...
            kill_counter = 0
            while (
                redis_send_client.get(config.prefix_heart_beat + ip)
                and kill_counter < 100
            ):
                time.sleep(0.1)
            ...
            if kill_counter < 100:
                ...
                return Response(
                    {
                        "status": 1,
                        "msg": util.safe_decode(_("close success")),
                        "delete_list": deletelist,
                    }
                )
            else:
                return Response(
                    {"status": -1, "msg": util.safe_decode(_("close failed"))}
                )
            ...
            redis_send_client.rpush(
                config.key_conn_control, ip + config.spliter + conn_uuid
            )

            conn_counter = 0
            while (
                not redis_send_client.get(config.prefix_heart_beat + ip)
                and conn_counter < 100
                and (not redis_log_client.hget(conn_uuid, "exit_code"))
            ):
                time.sleep(0.1)
            ...
            if exit_code == "0":
                return Response(
                    {"status": 1, "msg": util.safe_decode(_("connect success"))}
                )
            elif exit_code:
                ...
                return Response(
                    {"status": -2, "msg": util.safe_decode(_("connect timeout"))}
                )
