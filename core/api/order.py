...
class Order(baseview.BaseView):
    ...
    def post(self, request, args=None):
        ...
        if args == "select":
            ...
            redis_send_client.rpush(
                config.prefix_select + config.spliter + cmd_id, select_value
            )
            redis_send_client.delete(
                config.prefix_select + "_all" + config.spliter + cmd_id
            )
        ...
        if args == "download":
            ...
            all_log_str = (
                redis_log_client.hget(config.prefix_log + workid, "log") or "[]"
            )
            ...
            response["Content-Disposition"] = "%s;filename=%s" % (
                "attachment",
                escape_uri_path(name),
            )
            response["Access-Control-Expose-Headers"] = (
                "filename"  # 设置前端能获取到的header
            )
    ...
    def get(self, request, args=None):
        ...
        if args == "detail":
            ...
            def get_summary(work_id):
                ...
                if job_info:
                    for c in ast.literal_eval(job_info.get("log")):
                        ...
                        x["target"] = target_full.split(
                            config.spliter + x["target_id"]
                        )[0]
                        x["playbook_rownum"] = job_info.get("playbook_rownum")
                        x["exe_rownum"] = redis_log_client.llen(target_log)

                        log_target_sum = redis_log_client.hgetall(
                            "sum_" + x["target_id"]
                        )
                        ...
                        if x["begin_date"] and x["end_date"]:
                            x["endure"] = round(
                                float(x["end_date"]) - float(x["begin_date"]), 4
                            )
                        ...
                        if log_target_sum.get("stop_str"):
                            ...
                            last_cmd_id, last_step_info = get_list_key_info(
                                redis_log_client, target_log, "step"
                            )
                            x["exe_status"] = (
                                last_step_info if last_step_info else "executing"
                            )
                            x["finish"] = 0
            ...
            try:
                ...
                return Response(
                    {"status": -1, "msg": util.safe_decode(_("get log info failed"))}
                )
            ...
            if "rerun" in job_info and job_info["rerun"]:
                for _work_id in job_info["rerun"].split(",")[::-1]:
                    job_info_list.append(
                        redis_log_client.hgetall(config.prefix_log + _work_id)
                    )
            ...
            if not abort_time:
                ...
                redis_log_client.hset(
                    config.prefix_sum + target_id, "stop_str", "killing"
                )
                ...
                last_cmd_id, last_step_info = get_list_key_info(
                    redis_log_client, config.prefix_log_target + target_id, "step"
                )
                if last_step_info == "waiting select":
                    redis_send_client.rpush(
                        config.prefix_select + config.spliter + last_cmd_id, " "
                    )
                    redis_send_client.delete(
                        config.prefix_select + "_all" + config.spliter + last_cmd_id
                    )
                ...
                return Response(
                    {
                        ...
                        "msg": util.safe_decode(_("abort alread exist")),
                    }
                )
            ...
            if job_rerun:
                for j in job_rerun.split(","):
                    log_target_list += ast.literal_eval(
                        redis_log_client.hget("log_" + j, "log")
                    )
            ...
            for c in log_target_list:
                x = {}
                x["target"] = c[0].split(
                    config.spliter + c[0].split(config.spliter)[-1]
                )[0]
                x["target_id"] = c[0].split(config.spliter)[-1]
                last_uuid = redis_log_client.hget(
                    config.prefix_sum + x["target_id"], "last_uuid"
                )
